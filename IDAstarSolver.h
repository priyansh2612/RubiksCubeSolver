...
private:
    ...
    unordered_map<T, GenCube::MOVE, H> move_done;
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        while (!pq.empty()) {
            ...
            move_done[node.cube] = GenCube::MOVE(p.second);

            if (node.cube.isSolved()) return make_pair(node.cube, bound);
            node.depth++;
            for (int i = 0; i < 18; i++) {
                auto curr_move = GenCube::MOVE(i);
...
public:
    ...
    vector<GenCube::MOVE> solve() {
        ...
        while (!(curr_cube == rubiksCube)) {
            GenCube::MOVE curr_move = move_done[curr_cube];
