...
char GenCube::getColorLetter(COLOR color) {
//    switch (color) {
//        case COLOR::BLUE:
//            return 'B';
//        case COLOR::GREEN:
...
string GenCube::getMove(MOVE move) {
    switch (move) {
        case MOVE::L:
            return "L";
        case MOVE::LPRIME:
...
GenCube &GenCube::invert(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->lPrime();
        case MOVE::LPRIME:
...
vector<GenCube::MOVE> GenCube::randomShuffleCube(unsigned int times) {
...
string GenCube::getCornerColorString(uint8_t ind) const {
    string str = "";
...
uint8_t GenCube::getCornerIndex(uint8_t ind) const {
...
uint8_t GenCube::getCornerOrientation(uint8_t ind) const {
    string corner = getCornerColorString(ind);
